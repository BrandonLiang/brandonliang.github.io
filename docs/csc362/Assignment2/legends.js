...
*/
  ...
  */
  ...
  colorRangeArray.sort();
  
  var colorRangeAlt = toRange(d3.max(colorRangeArray)).values();
  
  var textLabel = d3.set();
  textLabel.add(colorScale.invertExtent("#fee5d9")[0]);
  textLabel.add(colorScale.invertExtent("#fee5d9")[1]);
  textLabel.add(colorScale.invertExtent("#fc9272")[0]);
  textLabel.add(colorScale.invertExtent("#fc9272")[1]);
  textLabel.add(colorScale.invertExtent("#ef3b2c")[0]);
  textLabel.add(colorScale.invertExtent("#ef3b2c")[1]);
  textLabel.add(colorScale.invertExtent("#99000d")[0]);
  textLabel.add(colorScale.invertExtent("#99000d")[1]);
  textLabelArray = textLabel.values();
...
/*TILES*/
 ...
 for (var i = 0; i < testsArray.length; i++) {
    ...
    svg.selectAll(".legend")
  	...
  	.attr("class","legend")
  	.append("rect")
  	.attr("x", function(d) {return 2*margin.left - 10 + 60*d;})
  	.attr("y", height)
  	.attr("width", 60)
  	.attr("height", 12)
   ...
   svg.selectAll(".legendText")
   	...
   	.attr("x",function(d) {return 2*margin.left - 12 + 60*d;})
   	.attr("y",height+20)
